...
namespace Hl7.Fhir.Model
{
	...
	public static class TagListExtensions
	{
		...
		public static void SetTextTag(this BundleEntry entry, string text)
		{
			...
			if (entry.Tags != null)
				result.AddRange(entry.Tags);

			result.RemoveAll(t => Equals(t.Scheme, Tag.FHIRTAGSCHEME_GENERAL) && (t.Term?.StartsWith(TAG_TERM_TEXT) ?? false));
		...
		{
			var textTag = entry.Tags
				.FilterByScheme(Tag.FHIRTAGSCHEME_GENERAL)
		...
		{
			List<Tag> result = new List<Tag>(bundle.Tags.Exclude(new[] { MESSAGE_TAG, DOCUMENT_TAG }));
		...
		{
			return entry.Tags.Where(t => Equals(t.Scheme, Tag.FHIRTAGSCHEME_PROFILE) && t.Term != null).Select(t => new Uri(t.Term));
		}
		...
		{
			return entry.Tags.Where(t => Equals(t.Scheme, Tag.FHIRTAGSCHEME_SECURITY) && t.Term != null).Select(t => new Uri(t.Term));
		}
