...
namespace Hl7.Fhir.Validation
{
	public abstract class ValidationAttribute : Attribute
	{
		...
		/// </remarks>
		protected ValidationAttribute() : this(() => DataAnnotationsResources.ValidationAttribute_ValidationError) { }
			...
			if (_errorMessageResourceType != null && !string.IsNullOrEmpty(_errorMessageResourceName))
			{
				...
				if (property == null)
				{
					throw new InvalidOperationException(
						string.Format(CultureInfo.CurrentCulture, DataAnnotationsResources.ValidationAttribute_ResourceTypeDoesNotHaveProperty, _errorMessageResourceType.FullName, _errorMessageResourceName));
				}
				if (property.PropertyType != typeof(string))
				{
					throw new InvalidOperationException(
						string.Format(CultureInfo.CurrentCulture, DataAnnotationsResources.ValidationAttribute_ResourcePropertyNotStringType, property.Name, _errorMessageResourceType.FullName));
				}
